  ...
  <div class="min-h-screen bg-gray-100 relative">
    ...
    <main class="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div class="bg-white rounded-2xl shadow p-6">
        ...
        <!-- Task List / states -->
        <div v-if="taskStore.loading && taskStore.tasks.length === 0" class="text-gray-500">
          Loading tasks…
        </div>

        <div v-else-if="taskStore.tasks.length === 0" class="text-gray-500">No tasks yet.</div>
    ...
    >
      <div class="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <input v-model="editingTitle" class="w-full p-2 border rounded mb-3" />
...
const isLoading = computed(() => !!auth.loading || !!taskStore.loading)
...
async function addTask() {
  ...
  if (!title) return
  try {
    const task = await taskStore.addTask({ title })
